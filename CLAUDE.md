...
After clone, if `graphify-out/graph.json` is missing, run `graphify update .` (AST-only, no API cost).
...
- Commit only the core graph outputs when they change: `graphify-out/graph.json`, `graphify-out/manifest.json`, and `graphify-out/GRAPH_REPORT.md` (plus `.graphify_labels.json` / `.graphify_root` if updated). Do not commit `graphify-out/cache/`, `graphify-out/graph.html`, or `graphify-out/.graphify_analysis.json` — those are local-only.
